...
4.  组合
    ...
    3. 40.Combination Sum II
        1. 集合中存在重复元素
        2. 每个元素只能使用一次
        3. 同一层不能出现相同值,只考虑当前层，已经包含了下一层所有情况，所以已经包含了这一层下一次又出现相同数的情况
    4. 216.Combination Sum III
        1. 有范围限制
    5. 78.Subsets
        1. 所有子集合
    6. 90.Subsets II
        1. 此处nums[i] == nums[i-1]  与 nums[i] == nums[i+1]是不同的，执行第一个值，跳过后边的值，还是跳过第一个值，执行后边的值
    7. 401.Binary Watch
        1. 使用bitset库
5. 二维平面上使用回溯法
    1. 79Word Search
        1. 依赖递归
        2. 一些重要手段进行总结，对以后有很大帮助
            1. 二维数组偏移量数组
            2. 已经访问过数组的标记
        3. note:attention in 数组不一定是m行n列数组，可能某一行长度不一样
6. floodfill算法-经典问题
